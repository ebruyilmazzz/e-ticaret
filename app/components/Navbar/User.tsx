"use client";
import type { User } from "@prisma/client"; // ✅ `type` olarak import edildi
        ...
        <div className="absolute w-[150px] top-10 bg-white shadow-lg right-0 p-2 rounded-md">
          {currentUser ? (
            <div className="space-y-1">
              <div onClick={() => router.push("/admin")} className="text-slate-600">
                Admin
              </div>
              <div onClick={() => menuFunc("logout")} className="text-slate-600">
                Logout
              </div>
            </div>
          ) : (
            <div>
              <div onClick={() => menuFunc("register")} className="text-slate-600">
                Register
              </div>
